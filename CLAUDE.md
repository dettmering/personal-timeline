...
- **main.go** — env config (`DB_PATH`, `LISTEN_ADDR`, `API_KEY`, `TZ`), embeds `static/`, registers routes, starts server
- **handlers.go** — HTTP API (`GET/POST /api/entries`, `PUT /api/entries/{id}`, `GET /api/health`); enforces API key for automated entries, character limits, timezone-aware date parsing

...
- **main.go** — env config (`DB_PATH`, `LISTEN_ADDR`, `API_KEY`, `TZ`, `WEBHOOK_URL`, `ENCRYPTION_KEY`), embeds `static/`, registers routes, starts server, runs the sealing and OTS-upgrade background loops. Boot order: `migrate` → `BackfillHashes` → `EncryptBackfill` → serve.

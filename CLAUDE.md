...
- **handlers.go** — HTTP API (`GET/POST /api/entries`, `PUT /api/entries/{id}`, `GET /api/hashtags`, `GET /api/health`); enforces API key for automated entries, character limits, timezone-aware date parsing; fires outbound webhook on entry creation
